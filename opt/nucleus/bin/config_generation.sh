...
wmm_enabled=1
wpa=2
wpa_passphrase=${AP_PASSWORD:-52235223}
...
# Calculate network prefixes from IP addresses
MESH_NETWORK=$(echo $MESH_IP | cut -d. -f1-3).0/24
BR_LAN_NETWORK=$(echo $BR_LAN_IP | cut -d. -f1-3).0/24
...
# Do not edit manually - changes will be overwritten
# Configuration variables sourced from /etc/nucleus/mesh.conf
# Variables used: MESH_IP, BR_LAN_IP
# ============================================================
...
# Redistribute local mesh network ($MESH_NETWORK)
redistribute ip $MESH_NETWORK allow

# Redistribute br-lan network ($BR_LAN_NETWORK)
redistribute ip $BR_LAN_NETWORK allow
